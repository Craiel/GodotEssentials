    ...
    where TDimension : IComparable<TDimension>
{
    ...
    /// <returns>The</returns>
    public KDTreeTuple<TDimension[], TNode>[] NearestNeighbors(TDimension[] point, int neighbors)
    {
        var nearestNeighborList = new KDTreePriorityList<int, double>(neighbors, true);
        ...
        return nearestNeighborList.ToResultSet<double, TDimension, TNode>(this);
    }
    ...
    public KDTreeTuple<TDimension[], TNode>[] RadialSearch(TDimension[] center, double radius, int neighboors = -1)
    {
        var nearestNeighbors = new KDTreePriorityList<int, double>(this.Count);
        if (neighboors == -1)
        {
        ...
        return nearestNeighbors.ToResultSet<double, TDimension, TNode>(this);
    }
